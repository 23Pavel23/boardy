...
const API = 'https://api.pablo52.ai-info.ru';
const POST_ID = 1;
    ...
    return (
        <div>
                ...
                <div key={item.id} className="card mb-2">
                    <div className="card-body">
                        ...
                        {editId === item.id ? (
                            ...
                            <div>
                                <p>{item.body}</p>
                                <button className="btn btn-sm btn-outline-secondary" onClick={() => { setEditId(item.id); setEditText(item.body); }}>✏️</button>
                                <button className="btn btn-sm btn-outline-danger ms-2" onClick={() => del(item.id)}>🗑️</button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="input-group mt-3">
                <input className="form-control" placeholder="Новый комментарий" value={text} onChange={e => setText(e.target.value)} />
