...
async def create_comment(
    ...
    async with conn.cursor() as cur:
        ...
        await cur.execute(
            "INSERT INTO comments (body, post_id, user_id) VALUES (%s,%s,%s)",
...
async def update_comment(
    ...
    async with conn.cursor() as cur:
        await cur.execute(
            "UPDATE comments SET body=%s WHERE id=%s AND user_id=%s",
            (data.body, comment_id, user["user_id"])
        )
        if cur.rowcount == 0:
            conn.close()
            raise HTTPException(status_code=404, detail="Комментарий не найден или не ваш")
        await conn.commit()
    conn.close()
