...
Сборка и деплой в отдельном файле потому что deploy.yml триггерится только на push в main, ci.yml — на каждый PR. Повтор lint/smoke/test в deploy.yml нужен чтобы даже прямой push в main прошёл проверки.
...
В практике 15 был build: ./src/boardy-laravel — сервер сам собирал образы. Теперь image: ghcr.io/... — сервер только скачивает готовый образ. Сборка ушла в CI.
![14-compose-image](screenshots/14-compose-image.png)

### Задание 15. SSH-ключ деплоя и секреты
Отдельный ключ для деплоя — не личный. Если утечёт: отзываем только deploy_key, личный доступ цел.
...
docker compose up -d
```
Не нужно ничего пересобирать — образ уже готов в GHCR.

### Задание 19. Безопасность пайплайна
1. Версию action лучше пинить по SHA потому что тег (v3) может быть переписан злоумышленником.
2. pull_request_target имеет доступ к секретам — недоверенный код из форка может украсть токены.
3. Self-hosted runner в публичном репо опасен — любой может открыть PR и выполнить код на вашем сервере.
